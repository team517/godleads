...
- NO abuses de los PDFs. Un PDF (copys o informe) SOLO cuando de verdad aporta y da mejor imagen: el cliente quiere ver TODOS los mensajes/copys, o pide un informe/resultados formales. Si es una duda puntual, una pregunta corta o algo que se resuelve mejor con palabras, RESPONDE con palabras — no mandes un PDF por cualquier mención.
  ...
  for (const m of msgs) {
   try {
    ...
    const system = (prof?.ai_reply_prompt || DEFAULT_ATENCION)
      ...
      + `\n\nRAZONA PRIMERO (campo "reasoning"): en 1-2 frases piensa con criterio qué opción da la mejor imagen de OnePulso (profesional y humana) y es lo más útil para el cliente ahora; valora si conviene una respuesta con palabras o de verdad aporta un PDF. LUEGO elige la acción coherente con ese razonamiento.
...
- "send_copys": SOLO si el cliente quiere ver/recibir el CONJUNTO de mensajes/copys de su campaña ("mándame TODOS los mensajes", "quiero verlos enteros", "pásame los copys") y un PDF ordenado con logo es claramente lo mejor. NO pegues los mensajes en el correo: se genera el PDF con los mensajes ACTUALES. En "reply" una frase corta de acompañamiento. Si nombra una campaña, ponla en "campaign"; si no, null = todas. Si solo pregunta por UN email o un detalle, NO uses send_copys → responde con "reply".
- "send_report": SOLO si el cliente pide de forma clara RESULTADOS/ANALÍTICAS/un informe formal de su campaña y un PDF con métricas es lo más adecuado. Para un "¿cómo va?" informal, valora si es mejor una respuesta humana breve ("reply") en vez de un informe.
...
REGLA CLAVE: por defecto RESPONDE con palabras (reply) bien escritas y humanas. Los PDFs (send_copys/send_report) son la EXCEPCIÓN, solo cuando de verdad aportan y dan mejor imagen — NUNCA por cualquier mención ni todo el rato. NO avises al equipo por cada correo: "escalate" ÚNICAMENTE para lo esencial. Si el remitente NO es un cliente conocido, usa SOLO "escalate" (si es esencial) o "ignore".`;
    const userMsg = (known
