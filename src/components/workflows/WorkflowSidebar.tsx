...
export function WorkflowSidebar({ onAddNode }: Props) {
  return (
    <div className="w-56 border-r border-border bg-card/50 overflow-y-auto p-3 space-y-4">
      <p className="text-[10.5px] font-semibold uppercase tracking-widest text-muted-foreground px-1">Nodos</p>
      {categories.map((cat) => (
        <div key={cat.key} className="space-y-1">
          <div className="flex items-center gap-2 px-1 mb-1.5">
            <div className={cn("h-2 w-2 rounded-full", cat.color)} />
            <span className="text-[11.5px] font-semibold text-muted-foreground">{cat.label}</span>
          </div>
          {NODE_TYPES.filter((n) => n.category === cat.key).map((nt) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-[13px] font-medium text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors"
              >
