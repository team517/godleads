...
import {
  LayoutDashboard, Mail, Send, Users, Inbox, BarChart3, Settings, LogOut, Brain, Shield, UsersRound, Workflow, Tv, Handshake,
} from "lucide-react";
...
export function AppSidebar({ isMobile, isOpen, onClose }: AppSidebarProps) {
  ...
  const sidebarClasses = cn(
    "fixed left-0 top-0 z-40 flex h-screen w-60 flex-col bg-sidebar text-sidebar-foreground border-r border-sidebar-border transition-transform duration-200",
  ...
  const NavItem = ({ item }: { item: typeof mainNav[0] }) => {
    ...
    return (
      <Link
        ...
        className={cn(
          "flex items-center gap-3 rounded-lg px-3 py-2 text-[13px] font-medium transition-all duration-150 relative group",
          isActive
            ? "bg-sidebar-accent text-sidebar-primary"
            : "text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent/50"
        )}
      >
        <item.icon className={cn("h-[18px] w-[18px] transition-colors", isActive ? "text-sidebar-primary" : "text-sidebar-foreground/40 group-hover:text-sidebar-foreground/70")} />
        {item.label}
    ...
    <aside className={sidebarClasses}>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-5">
        ...
        <div className="space-y-0.5">
          <p className="px-3 mb-2 text-[10px] font-semibold uppercase tracking-widest text-sidebar-foreground/30">Principal</p>
        ...
        <div className="space-y-0.5">
          <p className="px-3 mb-2 text-[10px] font-semibold uppercase tracking-widest text-sidebar-foreground/30">Herramientas</p>
        ...
        {isAdmin && (
          <div className="space-y-0.5">
            ...
            >
              <Shield className={cn("h-[18px] w-[18px]", location.pathname.startsWith("/admin") ? "text-sidebar-primary" : "text-sidebar-foreground/40")} />
              Panel Admin
            </Link>
          </div>
        )}
      ...
      <div className="border-t border-sidebar-border/50 p-3 space-y-2">
        ...
        >
          <Settings className="h-[18px] w-[18px] text-sidebar-foreground/40" />
          Configuración
        </Link>
        <button
          onClick={handleSoftExit}
          className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[13px] font-medium text-sidebar-foreground/40 hover:text-sidebar-foreground hover:bg-sidebar-accent/50 transition-all duration-150"
        >
        ...
        <button
          ...
          className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[13px] font-medium text-sidebar-foreground/40 hover:text-destructive hover:bg-destructive/10 transition-all duration-150"
        >
          <LogOut className="h-[18px] w-[18px]" />
          Cerrar sesión
        </button>
